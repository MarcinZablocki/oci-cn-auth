...
class Cert:
    ...
    def __init__(self, cert):
        try:
            ...
            print('Incorrect certificate', flush=True)
    ...
    def get_san(self):
        """ Return True in SAN information exists or False if missing """
    ...
    def cluster_info(self):
        """ Return Cluster Id information """
        try:
            san = self.cert.extensions.get_extension_for_oid(x509.oid.ExtensionOID.SUBJECT_ALTERNATIVE_NAME)
            ci = json.loads(b64decode(san.value.get_values_for_type(x509.OtherName)[0].value).decode('utf-8'))
            return ci['clusterId']
