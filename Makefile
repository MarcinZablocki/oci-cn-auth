...
FILES=--prefix / \
  ...
  .version-oci-cn-auth=/var/lib/oci-cn-auth/.version-oci-cn-auth
...
all: set_version_file generate_service_with_version $(EL7_RPM) $(EL8_RPM) $(PKG_DEB) $(SLES_RPM)
...
set_version_file:
	echo ${PKG_VERSION}-${PKG_RELEASE} > .version-oci-cn-auth

generate_service_with_version:
	set_version_in_service_file.sh
